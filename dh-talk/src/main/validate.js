import { MAX_BODY_CHARS, MAX_FILENAME_CHARS, MAX_FILE_BASE64_CHARS } from '../shared/limits.js';
...
export function validateInbound(msg) {
  ...
  switch (msg.kind) {
    case 'message': {
      ...
      if (msg.body.length > MAX_BODY_CHARS) return { ok: false, reason: 'body 길이 초과' };
    ...
    case 'file': {
      ...
      if (msg.filename.length > MAX_FILENAME_CHARS) return { ok: false, reason: 'filename 길이 초과' };
      ...
      if (msg.dataBase64.length > MAX_FILE_BASE64_CHARS) return { ok: false, reason: '파일 5MB 초과' };
      return { ok: true };
    }
