...
- [X] **vis-chart-02**: 수면 효율 (SE) 도넛 또는 progress ring → 측정: SVG circle stroke-dasharray
- [X] **vis-chart-03**: 척도 점수 변화 트렌드 (ISI/PHQ-9/ESS) 미니 스파크라인 → 측정: 각 척도 카드에 SVG path 미니 차트
- [X] **vis-chart-04**: 빈 데이터 시 차트 자리에 "7일 이상 기록 시 그래프가 나타납니다" 안내 → 측정: empty state 메시지

### 입력 단순화

- [X] **input-01**: 시각 입력 = 휠/스피너 (텍스트 입력 금지, iOS time picker 사용) → 측정: input[type="time"] 사용
- [X] **input-02**: 척도 응답 = 큰 버튼 5개 가로 (라디오 버튼 텍스트 금지) → 측정: 척도 응답 영역에 .scale-btn × 5
- [X] **input-03**: 다음 문항 자동 진행 (응답 즉시 다음 페이지, "다음" 버튼 불필요) → 측정: 응답 onclick 핸들러 = 다음 문항 자동 이동
- [X] **input-04**: 척도 진행 상황 표시 (5/14 같은 도트 또는 progress bar) → 측정: 척도 상단에 progress 요소

## Track 5: 점진적 공개 (5 항목)

- [X] **prog-01**: 1주차 사용자에겐 인지재구조 탭 비활성화 (잠금 아이콘 + "Week 2부터") → 측정: state.weekNumber < 2일 때 탭 disabled
...
- [X] **hyg-01 ~ hyg-30**: (기존 ROADMAP의 자동 항목 그대로, 우선순위 후순위)
