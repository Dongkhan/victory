...
<!-- alias: docs/ROADMAP.md = ROADMAP-v5 -->
...
- [X] **err-04**: schemaVersion 마이그레이션 — 5개 LEGACY_KEY 모두 처리 → 측정: 마이그레이션 함수 + 분기 검증
...
- [X] **i18n-02**: 영문 fallback — Intended Use / 면책 / 위기 안내 3섹션 → 측정: 영문 텍스트 존재
...
- [X] **ref-04**: 도구 출처 명시 — "본 도구 제작: 동행 정신건강의학과의원, 김포" → 측정: About에 명시
- [X] **ref-05**: 면책 정확 문구 — About에 "본 도구는 의료기기가 아니며, 진단 또는 처방을 대체하지 않습니다. 임상적 의사결정은 반드시 의료 전문가와 상의하세요." → 측정: About에 정확 매치

## Track 13: 개발자 위생 (6 항목)

- [X] **dev-01**: DEV_MODE 가드 외 UI dev 노출 0 → 측정: "debug" "dev" "test" 텍스트 UI 노출 0
- [X] **dev-02**: console.log 정리 (DEV_MODE 가드 외) → 측정: 정상 사용 시 console clean
- [X] **dev-03**: 단일 HTML 의존성 0 → 측정: package.json 없음
- [X] **dev-04**: 파일 크기 120KB 이하 (현재 61KB, 위 모든 작업 후 한계) → 측정: stat 120KB 이하
