...
- [X] **meta-02**: 모션 토큰 정의 — `--transition-fast` / `--transition-page` → 측정: `--transition-*` 변수 2개 이상
- [X] **meta-03**: 간격·라운드 스케일 토큰화 — `--space-*`, `--radius-*` → 측정: 각 스케일 변수 정의

## Track 1: 명백한 결함 — 즉시 수정 (6 항목)

- [X] **bug-01**: `100vh` -> `100dvh` 전환 (iOS Safari) — 현재 1건 잔존 → 측정: `100vh` 검색 0건
