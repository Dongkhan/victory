...
def test_latest_build_is_the_one_linked_everywhere():
    assert "run-quest/prototype/v0.4.html" in (ROOT / "index.html").read_text(encoding="utf-8")
    assert "./prototype/v0.4.html" in (ROOT / "run-quest/index.html").read_text(encoding="utf-8")
    ...
    for src in ("/run", "/running", "/run-quest",
                ...
                "/run-quest/prototype/v0.2.1.html", "/run-quest/prototype/v0.2.2.html",
                "/run-quest/prototype/v0.3.html"):
        assert targets.get(src) == "/run-quest/prototype/v0.4.html", src
