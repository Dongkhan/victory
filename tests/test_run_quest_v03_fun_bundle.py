...
def test_latest_build_is_the_one_linked_everywhere():
    assert "run-quest/prototype/v0.3.html" in (ROOT / "index.html").read_text(encoding="utf-8")
    assert "./prototype/v0.3.html" in (ROOT / "run-quest/index.html").read_text(encoding="utf-8")
    ...
    for src in ("/run", "/running", "/run-quest",
        ...
        assert targets.get(src) == "/run-quest/prototype/v0.3.html", src
